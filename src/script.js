...
const generateGalaxy = () =>
{
    ...
    for(let i = 0; i < parameters.count; i++)
    {
        ...
        // Randomness
        randomness[i3 + 0] = Math.pow(Math.random(), parameters.randomnessPower) * (Math.random() < 0.5 ? 1 : - 1) * parameters.randomness * radius
...
generateGalaxy()
gui.add(material.uniforms.uSize, 'value').min(1).max(10).step(0.01).name('particleSize').onChange((value) => {
    material.uniforms.uSize.value = value;
});
gui.add(material.uniforms.uSpeed, 'value').min(0).max(2).step(0.01).name('particleSpeed').onChange((value) => {
